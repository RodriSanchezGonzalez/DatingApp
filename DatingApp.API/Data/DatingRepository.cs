...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            {
                var userLikees = await GetUsersLikes(userParams.UserId, false);
        ...
        {
            var user = await this.context.Users.Include(x => x.Likers)
                                                    .Include(x => x.Likees)
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessagesParams messageParams)
        {
            var messages = this.context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
        ...
        {
            var messages = await this.context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
                                                .Include(u => u.Recipient).ThenInclude(p => p.Photos)
